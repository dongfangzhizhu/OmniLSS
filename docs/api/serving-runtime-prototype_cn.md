...
所有 HTTP metadata response 都包含 `X-Request-ID`；如果入站 request 提供 request ID，则会透传。嵌入式 deployment 也可以通过 `serve(..., event_sink=...)` 接收结构化 request event，用于 prototype-safe logging 或测试观测。
