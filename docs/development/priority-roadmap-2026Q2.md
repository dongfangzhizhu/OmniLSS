...
[中文版本](priority-roadmap-2026Q2_cn.md)
...
**当前状态（2026-05-15）**：🟢 API exposure done
...
- 已新增 `omnilss/src/omnilss/rs_jax.py`（`_jax_wls` 与 `_jax_penalized_wls`）及对应一致性测试，作为 T-02 的数值基础层。
- 已新增 `omnilss/src/omnilss/algorithms/cg_algorithm_v2.py` 并将 `cg_fit` 路由到 `cg_fit_v2`，作为 T-04 的 API 层推进。
- 以上不代表 T-02/T-04 已完成：RS 的 JAX 主循环与 CG 的独立跨导数外循环仍需实现。
