...
| 2 | 2026-05-24 to 2026-05-30 | D1/D2 prediction hardening | Legacy prediction entry points audited; public artifact-schema examples added | Complete for current runtime surface; continue auditing newly discovered aliases |
| 3 | 2026-05-31 to 2026-06-06 | D3 capability gates | Runtime capability matrix fully aligned with docs and strict routing | In progress; method-route map emitted and strict routing shares registry helper |
...
- Week 2 started only after the core Week 1 gate: public artifact-schema examples and a validator CLI are available, and the current legacy prediction runtime wrappers now delegate to schema-safe prediction. Week 3 has started with a shared method-route capability map in the runtime matrix and strict routing helper.

## Current Week 2/3 Evidence

- Legacy prediction runtime wrappers now call schema-safe `predict_params()` and consult the family `prediction` capability before producing mean, distribution, quantile, or interval outputs. See [month1-legacy-prediction-runtime-audit-2026-05-18.md](month1-legacy-prediction-runtime-audit-2026-05-18.md).
- The capability matrix now includes `method_routes`, and fitting strict-mode checks use the same `require_method_route()` helper exposed from `omnilss.family_capabilities`.
