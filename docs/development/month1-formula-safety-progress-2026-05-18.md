...
- Malicious or unsupported expressions raise deterministic `ValueError` failures without executing arbitrary Python code.
- Regression tests now cover direct allowlisted functions and explicit rejection paths for attribute calls, subscripts, lambdas, comprehensions, and non-allowlisted imports.
...
- Add structured formula error classes with fields for parameter, term, and reason.
