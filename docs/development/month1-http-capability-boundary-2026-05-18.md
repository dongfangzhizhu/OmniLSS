...
- Added tests that start the HTTP server on an ephemeral local port and verify both health and capability matrix responses.

## Remaining Work

- Add authentication, request IDs, structured logs, payload limits, and rate limits before exposing fit/predict HTTP endpoints.
