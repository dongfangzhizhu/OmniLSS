...
- JSON artifact 现在提供 validator，可检查 archive 结构、schema 版本、参数 schema 覆盖、系数/schema 一致性、smooth metadata 可用性，以及训练数据包含 warning；同时提供 capability snapshot compatibility report，用于对比保存时证据等级与当前 runtime registry。
