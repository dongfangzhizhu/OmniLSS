...
| 第 2 周 | 2026-05-24 至 2026-05-30 | D1/D2 预测加固 | 审计 legacy prediction 入口；补充公开 artifact-schema 示例 | 当前 runtime 表面已完成；继续审计后续发现的 alias |
| 第 3 周 | 2026-05-31 至 2026-06-06 | D3 能力门禁 | Runtime capability matrix 与文档和严格路由完全对齐 | 进行中；已输出 method-route map，strict routing 共享 registry helper |
...
- 第 2 周工作仅在第 1 周核心门禁完成后启动：公开 artifact-schema 示例和 validator CLI 已可用，当前 legacy prediction runtime wrapper 已委托给 schema-safe prediction。第 3 周已通过 runtime matrix 中的共享 method-route capability map 和 strict routing helper 启动。

## 当前第 2/3 周证据

- Legacy prediction runtime wrapper 现在调用 schema-safe `predict_params()`，并在输出 mean、distribution、quantile 或 interval 前查询 family `prediction` capability。参见 [month1-legacy-prediction-runtime-audit-2026-05-18_cn.md](month1-legacy-prediction-runtime-audit-2026-05-18_cn.md)。
- Capability matrix 现在包含 `method_routes`，拟合 strict-mode 检查使用从 `omnilss.family_capabilities` 暴露的同一个 `require_method_route()` helper。
