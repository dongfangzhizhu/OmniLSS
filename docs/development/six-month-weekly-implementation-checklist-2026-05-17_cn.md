...
| 第 3 周 | 2026-05-31 至 2026-06-06 | D3 能力门禁 | Runtime capability matrix 与文档和严格路由完全对齐 | 已启动；matrix artifact、HTTP/gRPC response、`gamlss()` strict routing 与 HTTP 和 gRPC service route-admission report 现在共享 method-routing contract |
...
- JSON artifact 现在提供带 typed issue severity 的 versioned validator report，可检查 archive 结构、schema 版本、参数 schema 覆盖、系数/schema 一致性、smooth metadata 可用性，以及训练数据包含 warning；同时提供 capability snapshot compatibility report，用于对比保存时证据等级与当前 runtime registry。
