...
| 第 3 周 | 2026-05-31 至 2026-06-06 | D3 能力门禁 | Runtime capability matrix 与文档和严格路由完全对齐 | 已启动；matrix artifact、HTTP/gRPC response、`gamlss()` strict routing 与 HTTP 和 gRPC service route-admission report 现在共享 method-routing contract；可导入的 `method_route_feature()` / `require_method_route()` helper、生成 JSON 的 `method_routes` alias、schema version 与 matrix validator 已同步 |
