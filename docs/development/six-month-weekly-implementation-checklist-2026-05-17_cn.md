...
| 第 2 周 | 2026-05-24 至 2026-05-30 | D1/D2 预测加固 | 审计 legacy prediction 入口；补充公开 artifact-schema 示例 | 进行中；已添加 artifact-schema 文档、validator CLI、legacy/indirect prediction 入口 schema-safe 审计、plot/report wrapper 传播测试、公开 error-envelope 示例和 gRPC prediction error envelope；HTTP metadata error envelope、POST payload-limit gate、prototype structured event hook 和 gRPC route-capability preflight 已添加 |
| 第 3 周 | 2026-05-31 至 2026-06-06 | D3 能力门禁 | Runtime capability matrix 与文档和严格路由完全对齐 | 当前第 1 月范围已完成；matrix artifact、HTTP/gRPC response、`gamlss()` strict routing、service route-admission report、可导入 helper API、schema version 与 matrix validator 已同步；validation 扩展和 async-job admission 已延期到后续计划工作 |
| 第 4 周 | 2026-06-07 至 2026-06-16 | 第 1 月 release gate | Core trust checkpoint 和可复现实验证据 | Offline gate 已完成；docs-localization、capability-matrix validation、artifact validation、linear JSON roundtrip prediction 与 schema-safe prediction-error smoke checks 已覆盖；package/build checks 仍取决于环境；详见 [第 1 月 release gate 预检](month1-release-gate-preflight-2026-05-18_cn.md) |
| 第 5 周 | 2026-06-17 至 2026-06-23 | D4 验证矩阵 | 优先 family validation schema 和首批 JSON 输出 | 已启动；已在 [第 2 月 family validation schema 种子](month2-family-validation-schema-2026-05-18_cn.md) 中添加优先 family validation schema seed 和 JSON plan |
