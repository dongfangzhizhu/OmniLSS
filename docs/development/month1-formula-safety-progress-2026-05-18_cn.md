...
- 恶意或不支持的表达式会以确定性的 `FormulaEvaluationError` 失败，不执行任意 Python 代码。
- `FormulaEvaluationError` 携带结构化的 `term` 和 `reason` 字段，便于诊断和服务错误映射。
- 回归测试覆盖了直接白名单函数、属性调用、下标访问、lambda、推导式和非白名单 import 的拒绝路径，以及结构化错误属性。

## D2 剩余工作

- 将 prediction 以及 smooth/tensor 参数处理中剩余的临时字符串拆分替换为共享的受限解析器工具。
- 将表达式级别的结构化公式错误从 `term`/`reason` 扩展到公开 API 错误，并包含分布参数名。
