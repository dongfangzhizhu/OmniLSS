...
- 矩阵现在包含 `schema` block，用于命名 payload（`omnilss.family_capability_matrix`）、重复 schema version，并标明 `method_capability_features` 是 canonical field，同时将 `method_routes` 列为 client migration 使用的 compatibility field。
