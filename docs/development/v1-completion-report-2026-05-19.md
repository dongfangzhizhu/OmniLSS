...
- **Phase 6 — Commercial Pro differentiation:** Pro AutoML ranks candidate families through Core client calls by deviance, AIC, BIC, and GAIC; Pro bootstrap confidence intervals use repeated Core fits; and Pro implementation/tests preserve the gRPC client boundary without importing `omnilss`.
...
4. Distribution family lookup is now registry-first with no retained legacy resolver chain in `distributions.py`.
