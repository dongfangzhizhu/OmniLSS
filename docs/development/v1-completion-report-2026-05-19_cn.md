...
- **第三阶段 — 统一分布注册表与验证：** `distribution_registry.py` 已成为权威查找入口，内置分布通过单一字典表注册，`resolve_family()` 委托给注册表，旧的 legacy if-chain resolver 表已替换为委托到注册表的兼容 shim，数值契约验证覆盖已就位。
...
- **第六阶段 — 商业 Pro 差异化：** Pro AutoML 通过 Core `BatchFit` client 调用按 deviance、AIC、BIC、GAIC 排名候选分布族；Pro bootstrap 置信区间使用 batch-fit 重采样；Pro 实现和测试继续保持 gRPC client 边界且不导入 `omnilss`。
...
4. 分布族查找现在以注册表为优先；历史私有 resolver 名称仅作为兼容 shim 保留，不再是独立 resolver chain。
