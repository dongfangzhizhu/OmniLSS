"""R/Python consistency tests for LO (Logistic) distribution.

Tests that OmniLSS produces results consistent with R GAMLSS
for the Logistic distribution family.

运行方式:
    python -m unittest tests.test_r_consistency_lo
    或
    PYTHONPATH=src JAX_PLATFORMS=cpu python -m unittest tests.test_r_consistency_lo -v
"""
...
from omnilss.distributions import LO
...
class TestLOConsistency(unittest.TestCase):
    """Test consistency between Python and R for LO distribution."""
    ...
    def test_intercept_only(self):
        ...
        loc = 5.0
        scale = 2.0
        y = logistic.rvs(loc=loc, scale=scale, size=n)
    ...
    def test_simple_covariate(self):
        ...
        mu = 3.0 + 2.0 * x
        scale = 1.5
        y = np.array([logistic.rvs(loc=mu_i, scale=scale) for mu_i in mu])
