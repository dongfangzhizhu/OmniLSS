"""R/Python consistency tests for WEI (Weibull) distribution.

Tests that OmniLSS produces results consistent with R GAMLSS
for the Weibull distribution family (2 parameters: mu, sigma).
"""
...
from omnilss.distributions import WEI
...
class TestWEIConsistency(unittest.TestCase):
    """Test consistency between Python and R for WEI distribution."""
    ...
    def test_simple_weibull_model(self):
        """Test simple Weibull model: y ~ x."""
        ...
        mu = np.exp(1.0 + 0.3 * x)
        sigma = 0.5
        # Weibull parameterization: shape = 1/sigma, scale = mu
        shape = 1 / sigma
        scale = mu
        y = np.random.weibull(shape, n) * scale
    ...
    def test_intercept_only_weibull(self):
        """Test intercept-only Weibull model: y ~ 1."""
        # Set random seed
        np.random.seed(123)
    ...
    def test_multiple_predictors_weibull(self):
        """Test Weibull model with multiple predictors: y ~ x1 + x2."""
        # Set random seed
        np.random.seed(456)
