"""GU (Gumbel) R/Python consistency tests.

Tests OmniLSS GU distribution against R gamlss.dist::GU.
...
Run:
    PYTHONPATH=src JAX_PLATFORMS=cpu python -m unittest tests.test_r_consistency_gu -v
"""
...
from omnilss.distributions_b1 import GU
...
class TestGUConsistency(unittest.TestCase):
    """GU distribution R/Python consistency tests."""
    ...
    def test_intercept_only_gu(self):
        """Test intercept-only GU model: y ~ 1."""
        np.random.seed(42)
        n = 100
        
        # Generate data from Gumbel distribution
        # Using scipy's gumbel_r (right-skewed Gumbel)
        from scipy.stats import gumbel_r
        mu_true = 5.0
        sigma_true = 2.0
        y = gumbel_r.rvs(loc=mu_true, scale=sigma_true, size=n)
    ...
    def test_simple_gu_model(self):
        """Test simple GU model: y ~ x."""
        np.random.seed(123)
        n = 100
        ...
        mu_true = 2.0 + 0.5 * x
        sigma_true = 1.5
        y = gumbel_r.rvs(loc=mu_true, scale=sigma_true)
    ...
    def test_multiple_predictors_gu(self):
        """Test GU model with multiple predictors: y ~ x1 + x2."""
        np.random.seed(456)
        n = 100
