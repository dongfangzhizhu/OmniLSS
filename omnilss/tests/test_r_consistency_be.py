"""R/Python consistency tests for BE (Beta) distribution.

Tests that OmniLSS produces results consistent with R GAMLSS
for the Beta distribution family (2 parameters: mu, sigma).
"""
...
from omnilss.distributions import BE
...
class TestBEConsistency(unittest.TestCase):
    """Test consistency between Python and R for BE distribution."""
    ...
    def test_intercept_only_beta(self):
        """Test intercept-only Beta model: y ~ 1."""
        # Set random seed
        np.random.seed(42)
    ...
    def test_simple_beta_model(self):
        """Test simple Beta model: y ~ x."""
        ...
        # Generate Beta data
        # logit(mu) = 0.5 + 0.3 * x
        mu = 1 / (1 + np.exp(-(0.5 + 0.3 * x)))
        sigma = 0.15
        y = np.array([np.random.beta(m / sigma, (1 - m) / sigma) for m in mu])
    ...
    def test_multiple_predictors_beta(self):
        """Test Beta model with multiple predictors: y ~ x1 + x2."""
        # Set random seed
        np.random.seed(456)
        ...
        # Generate Beta data
        # logit(mu) = 0.2 + 0.4 * x1 - 0.3 * x2
        mu = 1 / (1 + np.exp(-(0.2 + 0.4 * x1 - 0.3 * x2)))
        sigma = 0.2
        y = np.array([np.random.beta(m / sigma, (1 - m) / sigma) for m in mu])
