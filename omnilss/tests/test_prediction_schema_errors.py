...
def _smooth_schema_broken_model(seed: int, tmp_path):
    rng = np.random.default_rng(seed)
    x = np.linspace(0.0, 1.0, 50)
    y = np.sin(2 * np.pi * x) + rng.normal(scale=0.05, size=len(x))
    model = gamlss("y ~ pb(x)", family="NO", data={"y": y, "x": x}, max_iter=2)

    path = tmp_path / f"smooth-broken-{seed}.omnilss"
    save_model_json(model, path)
    loaded = load_model_json(path)
    loaded.additional_slots = {**loaded.additional_slots, "smooth_infos": {}}
    return loaded
...
def test_scoring_wrapper_reuses_schema_safe_prediction_errors(tmp_path):
    from omnilss.scoring import log_score

    loaded = _smooth_schema_broken_model(35, tmp_path)

    with pytest.raises(PredictionSchemaError) as exc_info:
        log_score(loaded, {"x": np.array([0.1, 0.2])}, np.array([0.0, 0.0]))

    assert exc_info.value.code == "missing_smooth_metadata"


def test_get_tgd_reuses_schema_safe_prediction_errors(tmp_path):
    from omnilss.gamlssVGD_23_12_21 import get_tgd_data

    loaded = _smooth_schema_broken_model(36, tmp_path)

    with pytest.raises(PredictionSchemaError) as exc_info:
        get_tgd_data(
            loaded,
            newdata={"y": np.array([0.0, 0.0]), "x": np.array([0.1, 0.2])},
        )

    assert exc_info.value.code == "missing_smooth_metadata"


def test_extract_tgd_reuses_schema_safe_prediction_errors(tmp_path):
    from omnilss.stepTGD import extract_tgd_data

    loaded = _smooth_schema_broken_model(37, tmp_path)
