...
def test_solve_joint_system_matches_numpy_reference():
    ...
    np.testing.assert_allclose(np.asarray(delta["mu"]), expected[:2], rtol=1e-10, atol=1e-10)
    np.testing.assert_allclose(np.asarray(delta["sigma"]), expected[2:], rtol=1e-10, atol=1e-10)
