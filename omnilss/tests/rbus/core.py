...
class RTestBus:
        ...
        try:
            ...
            subprocess.run(
                [
                    "Rscript",
                    "-e",
                    "library(jsonlite); library(gamlss); library(gamlss.dist)",
                ],
                check=True,
                capture_output=True,
                text=True,
            ...
            logger.warning("Rscript or required R packages are unavailable.")
            raise RuntimeError(
                "Rscript with jsonlite, gamlss, and gamlss.dist is required"
            ) from exc
    ...
    def _run_script(self, script_name: str, **kwargs) -> Dict[str, Any]:
        ...
        if not script_path.exists():
            raise FileNotFoundError(
                f"R script {script_name} not found in {self.scripts_dir}"
            )
        ...
        try:
            ...
            if json_str is None:
                raise ValueError(
                    f"Could not find JSON output in R script stdout: {result.stdout}"
                )
    ...
    def eval_family(
        self, family: str, func_type: str, args: Dict[str, Any]
    ) -> np.ndarray:
        ...
        result = self._run_script(
            "eval_family.R", family=family, type=func_type, args=args
        )
        if "error" in result:
