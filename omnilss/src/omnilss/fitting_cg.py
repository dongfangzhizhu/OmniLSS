...
def fit_cg(
    ...
    def log_likelihood(params_dict, data_dict):
        return _compute_log_likelihood(
            params_dict, data_dict, family, design_matrices
        )
    ...
    for iteration in range(max_iter):
        ...
        if iteration > 0:
            ...
            if dev_change < tol:
                ...
                fitted_values = _compute_fitted_values(
                    params, design_matrices, family
                )
    ...
    if verbose:
        ...
        if len(deviance_history) > 1:
            print(f"Final deviance change: {abs(deviance_history[-1] - deviance_history[-2]):.2e}")
        print("="*70)
    
    warnings.warn(
        f"CG algorithm did not converge after {max_iter} iterations",
        RuntimeWarning
    )
...
def _compute_log_likelihood(
    ...
    if hasattr(family, 'log_likelihood'):
        ...
        if hasattr(family, 'd'):
            ...
            raise AttributeError(f"Family {family.name} has no log_likelihood or d method")
...
def _compute_expected_fisher(
    ...
    if "beta_sigma" in params and X_sigma is not None and sigma is not None:
        ...
        elif hasattr(family, 'hessian_functions') and 'sigma' in family.hessian_functions:
    ...
    if len(blocks) == 1:
        ...
        for block in blocks:
            size = block.shape[0]
            fisher = fisher.at[start_idx:start_idx+size, start_idx:start_idx+size].set(block)
            start_idx += size
