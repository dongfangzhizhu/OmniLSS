...
def _resolve_family_legacy(family_name: str) -> FamilyDefinition:
    ...
    # Batch 1 AD Families
    if family_name == "GU":
        from .distributions_b1 import GU
        return GU()
    if family_name == "RG":
        from .distributions_b1 import RG
        return RG()
    if family_name == "IGAMMA":
        from .distributions_b1 import IGAMMA
        return IGAMMA()
    if family_name == "PARETO2":
        from .distributions_b1 import PARETO2
        return PARETO2()
    if family_name == "NBII":
        from .distributions_b1 import NBII
        return NBII()

    # Batch 2 AD Families
    if family_name == "NO2":
        from .distributions_b2 import NO2
        return NO2()
    if family_name == "LOGNO2":
        from .distributions_b2 import LOGNO2
        return LOGNO2()
    if family_name == "PE":
        from .distributions_b2 import PE
        return PE()
    if family_name == "SIMPLEX":
        from .distributions_b2 import SIMPLEX
        return SIMPLEX()
    if family_name == "EXGAUS":
        from .distributions_b2 import exGAUS
        return exGAUS()

    # Batch 3: Heavy-tail / Skewed families
    if family_name == "SHASH":
        from .distributions_b3 import SHASH
        return SHASH()
    if family_name == "SHASHO":
        from .distributions_b3 import SHASHo
        return SHASHo()
    if family_name == "SN1":
        from .distributions_b3 import SN1
        return SN1()
    if family_name == "SN2":
        from .distributions_b3 import SN2
        return SN2()
    if family_name == "GT":
        from .distributions_b3 import GT
        return GT()

    # Batch 4: Beta-inflated families
    if family_name == "BEINF":
        from .distributions_b4 import BEINF
        return BEINF()
    if family_name == "BEINF0":
        from .distributions_b4 import BEINF0
        return BEINF0()
    if family_name == "BEINF1":
        from .distributions_b4 import BEINF1
        return BEINF1()
    if family_name == "BEZI":
        from .distributions_b4 import BEZI
        return BEZI()
    if family_name == "BEOI":
        from .distributions_b4 import BEOI
        return BEOI()

    # Batch 5: Zero-inflated / Zero-altered families
    if family_name == "ZAGA":
        from .distributions_b5 import ZAGA
        return ZAGA()
    if family_name == "ZAIG":
        from .distributions_b5 import ZAIG
        return ZAIG()
    if family_name == "ZIP2":
        from .distributions_b5 import ZIP2
        return ZIP2()
    if family_name == "ZINBI":
        from .distributions_b5 import ZINBI
        return ZINBI()
    if family_name == "ZAP":
        from .distributions_b5 import ZAP
        return ZAP()
    if family_name == "ZIBI":
        from .distributions_b10_zero_variants import ZIBI
        return ZIBI()

    # Batch 6: Discrete special series
    if family_name == "PIG":
        from .distributions_b6 import PIG
        return PIG()
    if family_name == "SICHEL":
        from .distributions_b6 import SICHEL
        return SICHEL()
    if family_name == "SI":
        from .distributions_b6 import SI
        return SI()
    if family_name == "DPO":
        from .distributions_b6 import DPO
        return DPO()
    if family_name == "DEL":
        from .distributions_b6 import DEL
        return DEL()
    if family_name == "YULE":
        from .distributions_b6 import YULE
        return YULE()
    if family_name == "WARING":
        from .distributions_b6 import WARING
        return WARING()

    # Batch 7: Multinomial and compound families
    if family_name == "BB":
        from .distributions_b7 import BB
        return BB()
    if family_name == "BNB":
        from .distributions_b7 import BNB
        return BNB()
    if family_name == "MN3":
        from .distributions_b7 import MN3
        return MN3()
    if family_name == "MN4":
        from .distributions_b7 import MN4
        return MN4()
    if family_name == "MN5":
        from .distributions_b7 import MN5
        return MN5()

    # Batch 8: Additional continuous distributions
    if family_name == "GG":
        from .distributions_b8 import GG
        return GG()
    if family_name == "GB2":
        from .distributions_b8 import GB2
        return GB2()
    if family_name == "PARETO":
        from .distributions_b8 import PARETO
        return PARETO()
    if family_name == "NET":
        from .distributions_b8 import NET
        return NET()
    if family_name == "LNO":
